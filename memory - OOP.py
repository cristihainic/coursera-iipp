...
class Tile:
    ...
    def draw_tile(self, canvas):
        ...
        if self.exposed:
            ...
            tile_corners = (loc, [loc[0] + TILE_WIDTH, loc[1]],
...
def mouseclick(pos):
    global state, turns, first_exposed_tile, second_exposed_tile
    ...
    if state == 0:
        state = 1
        first_exposed_tile = clicked_tile
    elif state == 1:
        state = 2
        second_exposed_tile = clicked_tile
        turns += 1
        label.set_text("Turns = " + str(turns)) 
    else:
        if first_exposed_tile.get_number() != second_exposed_tile.get_number():               
            first_exposed_tile.hide_tile()
            second_exposed_tile.hide_tile()
        state = 1
        first_exposed_tile = clicked_tile
