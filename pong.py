...
def spawn_ball(direction):
    ...
    hor_vel = random.randrange(120.0, 240.0) / 60.0
    ver_vel = random.randrange(60.0, 180.0) / 60.0
...
def draw(canvas):
    ...
    #collide & reflect off top & bottom
    if ball_pos[1] <= BALL_RADIUS:
        ball_vel[1] = - ball_vel[1]
    elif ball_pos[1] >= (HEIGHT - 1) - BALL_RADIUS:
        ball_vel[1] = - ball_vel[1]
        
    #spawn ball if it touches gutters
    ...
    # draw scores
    canvas.draw_text(str(score1), [190, 30], 30, 'White')
    canvas.draw_text(str(score2), [390, 30], 30, 'White')
...
frame.add_button("Restart", restart, 66)
